...
var Schema = function (db, options) {
  ...
  var load = function (callback) {
    ...
    // / views option
    if (options.views) {
      _tasks.push('matviews')

      _orchestrator.add('matviews', function (next) {
        // / retrieve views data
        getMatViews(function (err, matviews) {
          if (!err) {
            __data.matviews = matviews
          }
          next(err)
        })
      })
  ...
  var getViews = function (callback) {
    ...
    /**
     * Retrieve all materialize views
     * @public
     * @function
     * @param {function} callback Callback function
     */
    var getMatViews = function (callback) {
        log.verbose('log', 'loading views ...')
        db.query('SELECT matviewname AS name, ' +
            'definition AS source ' +
            'FROM pg_matviews ' +
            'WHERE schemaname != ALL($1)',
            ['{information_schema,pg_catalog}'],
