...
var Delta = function (schema1, schema2, options) {
   ...
   * @function
   */
  var __compareViews = function (matView = false) {
    var _views1 = schema1.views
    var _views2 = schema2.views
    var _sqls = []

    const prefix = matView ? "MATERIALIZED " : ""
      ...
      for (j in _views2) {
        ...
        if (_view1.name === _view2.name) {
          ...
          if (_view1.source !== _view2.source) {
            _sqls.push('DROP ' + prefix + 'VIEW ' + _view1.name + ';')
            _sqls.push('CREATE ' + prefix + 'VIEW ' + _view1.name + ' AS\n' + _view1.source)
          }
          _exists = true
          break
        }
    ...
    if (options.mode === 'full') {
      for (i in _views2) {
        ...
        if (!_exists) {
          _sqls.push('DROP ' + prefix + 'VIEW ' + _view2.name)
        }
      }
    }

    const deltaView = matView ? __delta.matviews : __delta.views
    const msg = matView ? '--materialized views match' : '--views match'
    if (_sqls.length > 0) {
      deltaView.push(_sqls.join(';\n') + ';')
    } else {
      deltaView.push(msg)
    }
  }
